...
async function run() {
    ...
    try {
        ...
        // Pull and parse config file ('reviewers.yml') from main branch of main repository using only isomorphic-git (no fs)
        let config = undefined as { [key: string]: string[] } | undefined;
        try {
            const configFilePath = core.getInput("config") || "reviewers.yml";
